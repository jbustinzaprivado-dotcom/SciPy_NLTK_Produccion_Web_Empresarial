...
export default function Interpolacion() {
  ...
  return (
    <div className="page-shell" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {data && (
        <>
          <p style={{ fontSize: '0.82rem', color: '#52676b' }}>
            <code style={badge}>{data.metodo}</code> {data.explicacion}
          </p>

          {data.puntos.length === 0 ? (
            <p style={{ fontSize: '0.85rem', color: '#52676b' }}>No hay suficientes datos en este rango.</p>
          ) : (
            <div style={card}>
                  ...
                  <tbody>
                    {data.puntos.map((p) => (
                      <tr key={p.fecha} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={td}>
                          <span style={{
                            fontSize: '0.72rem', padding: '0.15rem 0.5rem', borderRadius: '8px',
...
const input: React.CSSProperties     = { padding: '0.45rem 0.6rem', border: '1px solid #cbd5e1', borderRadius: '8px', fontSize: '0.85rem', width: '120px', boxSizing: 'border-box' };
...
const badge: React.CSSProperties     = { fontSize: '0.7rem', color: '#52676b', fontFamily: 'monospace', backgroundColor: '#f1f5f9', padding: '0.2rem 0.5rem', borderRadius: '8px', marginRight: '0.4rem' };
